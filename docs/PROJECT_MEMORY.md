...
- Imagens do conjunto atual: 708 originais WebP e 2.124 variantes no Supabase, totalizando 2.832 caminhos ativos; quatro objetos anteriores de `pets-19` e oito objetos de `animes-027`/`animes-083` permanecem preservados para rollback, total físico de 2.844 objetos.
...
- O build gera 691 HTMLs de produto, 17 HTMLs de coleção, sitemap com 711 URLs e robots bloqueando `/admin`; HTML estático contém canonical, Open Graph, JSON-LD e conteúdo rastreável.
...
- Qualidade atual: 58 testes, 18 E2E/axe por perfil, typecheck e build; Lighthouse atual: home P98/A98, coleção P98/A100 e produto P91/A100, com Boas Práticas/SEO 100.
...
- Após a revisão corretiva de Animes, o catálogo remoto possui 691 produtos, 691 relações e 2.764 associações de imagem. O conjunto atual referencia 2.832 caminhos; o Storage contém 2.844 objetos físicos porque preserva quatro WebPs antigos de `pets-19` e oito de `animes-027`/`animes-083` para rollback.
