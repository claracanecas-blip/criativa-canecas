...
Atualizada em 20 de agosto de 2026. Este arquivo preserva contexto operacional e decisões entre sessões. Não deve conter tokens, senhas ou chaves privadas.
...
- Imagens do conjunto atual: 610 originais WebP e 1.830 variantes verificadas no Supabase, totalizando 2.440 caminhos; quatro objetos anteriores de `pets-19` permanecem preservados para rollback, total físico de 2.444 objetos.
...
- O build gera 593 HTMLs de produto, 17 HTMLs de coleção, sitemap com 613 URLs e robots bloqueando `/admin`; HTML estático contém canonical, Open Graph, JSON-LD e conteúdo rastreável.
...
- Base técnica da Fase 8 concluída: `/informacoes` está no rodapé, prerender e sitemap de 613 URLs; textos evitam promessas antes da confirmação no atendimento.
...
- Qualidade atual: 53 testes, 17 E2E/axe por perfil, typecheck e build aprovados; Lighthouse atual: home P98/A98, coleção P98/A100 e produto P91/A100, com Boas Práticas/SEO 100.
...
- Após as expansões de Pets, Futebol, Profissões, Religião e Divertidas, o catálogo remoto atual possui 593 produtos, 593 relações e 2.372 associações de imagem. RLS positiva/negativa, `db lint`, os 2.440 caminhos do conjunto atual, typecheck, build e navegação paginada foram validados; o Storage contém 2.444 objetos físicos devido ao rollback preservado de `pets-19`.
