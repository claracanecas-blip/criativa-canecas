...
- A coleção vazia `Personalizadas` foi retirada do catálogo público em 30 de agosto de 2026, sem categoria substituta, porque o mesmo caminho comercial já existe como a ação `Personalize do seu jeito`. A home agora destaca cinco categorias reais; `/colecao/personalizada` redireciona para `/personalizada`, e o fallback TypeScript também impede a reintrodução do atalho. A migration remota `20260830123000` marcou a coleção como não publicada e não listada após confirmar zero relações, sem excluir dados, e possui rollback versionado. A leitura pública positiva permaneceu disponível, a coleção oculta retornou zero linhas e a escrita anônima foi negada com HTTP `401`. O aceite local aprovou 76 testes, typecheck, build com 16 coleções/740 URLs, 124 cenários Playwright/Axe nos quatro perfis e inspeção visual desktop. Evidência em `docs/baselines/2026-08-30-personalization-category-cleanup/`.
