...
- O build gera 721 HTMLs de produto, 16 HTMLs de coleção, sitemap com 740 URLs e robots bloqueando `/admin`; HTML estático contém canonical, Open Graph, JSON-LD e conteúdo rastreável.
...
- O refinamento visual do catálogo de 30 de agosto de 2026 dá identidade própria ao topo das coleções, reúne contagem e filtros em uma barra compacta, melhora a leitura e os CTAs dos cards em duas colunas no celular e impede que o WhatsApp flutuante cubra grades móveis de coleção e busca. Títulos longos de produto passaram a quebrar sem overflow. Não houve alteração de banco, catálogo, preço ou mensagem comercial. A validação local aprovou typecheck, 73 testes, build com 741 URLs, 120 cenários E2E/Axe nos quatro perfis principais, inspeção em 1440 × 1000 e 390 × 844 e Lighthouse home P99/A100, coleção P94/A98 e produto P91/A100, com Boas Práticas/SEO 100. O PR `#9` foi mesclado por `f2630dd`; o CI `33318994130` aprovou 120 cenários principais, 30 no Edge e Lighthouse, a Vercel publicou o deployment `dpl_9myA5GtA5zo99KYjkKwyTHAcgpDc` e o smoke oficial confirmou status `200`, canonicals, duas colunas, CTAs compactos, WhatsApp contextual e ausência de overflow. Evidência em `docs/baselines/2026-08-30-catalog-mobile-polish/`.
- A coleção vazia `Personalizadas` foi retirada do catálogo público em 30 de agosto de 2026, sem categoria substituta, porque o mesmo caminho comercial já existe como a ação `Personalize do seu jeito`. A home agora destaca cinco categorias reais; `/colecao/personalizada` redireciona para `/personalizada`, e o fallback TypeScript também impede a reintrodução do atalho. A migration remota `20260830123000` marcou a coleção como não publicada e não listada após confirmar zero relações, sem excluir dados, e possui rollback versionado. A leitura pública positiva permaneceu disponível, a coleção oculta retornou zero linhas e a escrita anônima foi negada com HTTP `401`. O aceite local aprovou 76 testes, typecheck, build com 16 coleções/740 URLs, 124 cenários Playwright/Axe nos quatro perfis e inspeção visual desktop. O PR `#10` foi mesclado por `c9e1397`; o CI da `main` `33321308197` aprovou 124 cenários principais, 31 no Edge e Lighthouse P98/A100 na home, e a Vercel publicou `dpl_7A3Ju7EgabW1NAoEKrazkJsoqv6R`. O smoke oficial confirmou cinco categorias em desktop/celular, ação de personalização preservada, redirecionamento legado, canonical, ausência de overflow e sitemap com 740 URLs. Evidência em `docs/baselines/2026-08-30-personalization-category-cleanup/`.
...
48. Em grades de catálogo no celular, priorizar os CTAs próprios de cada produto e ocultar o atalho flutuante redundante do WhatsApp para não cobrir conteúdo; o atalho continua disponível nas demais páginas.
