...
- Imagens do conjunto atual: 710 originais WebP e 2.130 variantes verificadas no Supabase, totalizando 2.840 caminhos; quatro objetos anteriores de `pets-19` permanecem preservados para rollback, total físico de 2.844 objetos.
...
- O build gera 693 HTMLs de produto, 17 HTMLs de coleção, sitemap com 713 URLs e robots bloqueando `/admin`; HTML estático contém canonical, Open Graph, JSON-LD e conteúdo rastreável.
...
- Qualidade atual: 57 testes, 18 E2E/axe por perfil, typecheck e build aprovados; Lighthouse atual: home P98/A98, coleção P98/A100 e produto P91/A100, com Boas Práticas/SEO 100.
...
- Após as expansões de Pets, Futebol, Profissões, Religião, Divertidas e Animes, o catálogo remoto atual possui 693 produtos, 693 relações e 2.772 associações de imagem. RLS positiva/negativa, `db lint`, os 2.840 caminhos do conjunto atual, typecheck, build e navegação paginada foram validados; o Storage contém 2.844 objetos físicos devido ao rollback preservado de `pets-19`.
...
- A coleção `Animes` recebeu em 21 de agosto de 2026 100 produtos (`animes-001` a `animes-100`) de 36 temas, sem alterar os 43 modelos anteriores. O inventário local encontrou 2.301 imagens raster; arquivos genéricos foram identificados visualmente, a seleção respeita no máximo cinco artes novas por anime e os 100 mockups 1254 × 1254 foram revisados em folhas de contato. Cinco fontes de Pokémon foram bloqueadas pelo gerador e substituídas sem contornar o bloqueio. A migration remota `20260821190000` publicou 400 objetos WebP e levou o catálogo a 693 produtos; evidência em `docs/baselines/2026-08-21-anime-catalog/`.
