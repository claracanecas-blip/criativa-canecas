...
- Uma decisão posterior do proprietário simplificou o PR `#8` para um único atendimento assistido: o cliente escolhe a imagem original, informa frase e orientações e pede que a Criativa Canecas produza o mockup para aprovação. A prévia 3D, o editor, a exportação do gabarito e as dependências Three.js foram removidos. Em navegadores compatíveis, `Compartilhar foto e pedido` abre o compartilhamento nativo com o arquivo original; o cliente escolhe o WhatsApp. Como links `wa.me` não anexam arquivos, o fallback abre a conversa com a mensagem preenchida e orienta anexar a imagem manualmente. O arquivo permanece somente no navegador até uma ação explícita do cliente.
