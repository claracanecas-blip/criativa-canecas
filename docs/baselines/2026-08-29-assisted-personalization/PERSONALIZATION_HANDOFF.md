...
Em 30 de agosto, os dois níveis e preços viraram cartões de escolha no início da página, antes de qualquer campo ou passo. A apresentação e os quatro passos genéricos repetidos foram removidos. Arte pronta mostra apenas observação opcional, envia somente `R$ 39,90` e informa que não precisa de criação/adaptação; criação/adaptação coleta frase/ideia, envia somente `R$ 44,90` e pede composição e mockup para aprovação. Resumo, passos, CTA, rótulo do arquivo e orientação de anexo acompanham a escolha. `Caneca Mágica` e `Canecas com Foto` saíram da barra e do mega menu, preservando as rotas antigas. O commit `af15594` passou localmente em typecheck, 70 testes, build de 741 URLs, inspeção desktop/celular e `112/112` cenários E2E/Axe; o CI remoto aprovou `28/28` no Edge e Lighthouse. A Vercel concluiu a produção, e o smoke no domínio oficial confirmou status `200`, navegação simplificada, mensagens com apenas o valor escolhido e WhatsApp oficial.
