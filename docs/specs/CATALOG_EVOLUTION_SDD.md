...
- **29 de agosto de 2026 — fluxo híbrido, conferência guiada e handoff ao WhatsApp:** `/personalizada` passou a renderizar uma caneca procedural com Three.js e aplicar imagem/frase por textura de canvas mantida somente no navegador. `Quero ver e ajustar` centraliza a foto automaticamente e apresenta `Ver na caneca` como próxima ação, com rolagem/foco na conferência 3D; `Ajustar foto` abre e fecha explicitamente o editor, cujos controles de arraste, teclado, zoom e presets atualizam o 3D. `Quero que vocês criem` coleta foto, frase e orientação, trata o 3D como referência e leva ao WhatsApp um pedido explícito para a equipe preparar o mockup, sem exigir download. O editor usa o gabarito operacional confirmado de `21 × 8,7 cm`, sinaliza resolução estimada e, no caminho ajustável, exporta PNG de `2480 × 1028 px` com metadado de `300 dpi`; a mensagem pede prévia + original. No assistido, pede somente a foto original e registra que a composição poderá ser proposta pela equipe. Falta de WebGL 2 aciona fallback 2D. A oferta permanece limitada a `Caneca personalizada` e `Caneca personalizada com foto`. O aceite local cobriu 77 testes, typecheck, build com 741 URLs, seis cenários direcionados Playwright/Axe em Chrome e Safari móvel — incluindo os dois caminhos, conferência, editor, dimensões e densidade do download —, inspeção desktop/celular e teste privado com arte real do acervo sem copiá-la nem enviá-la. O PR em rascunho `#8` mantém preview Vercel protegido por autenticação, sem alterar `main`, produção, banco ou Storage; rollback consiste em restaurar a prévia 2D e retirar fluxo híbrido, editor, exportação e dependências 3D.
