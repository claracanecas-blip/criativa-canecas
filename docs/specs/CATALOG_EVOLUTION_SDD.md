...
**Entregas:** zoom acessível de 2,4× recortado na própria caixa e galeria preparada para múltiplas imagens; bloco seguro de detalhes/cuidados; depoimentos moderados no contexto do produto; filtros por tema, coleção e preço com ordenação; prévia local evoluída para caneca 3D com textura dinâmica de imagem/frase, giro por mouse, toque, teclado e botões, zoom e fallback 2D; fluxo híbrido com `Quero ver e ajustar`, que amplia automaticamente a foto em até `140%` quando isso melhora o preenchimento, anuncia a prévia pronta e oferece `Ver na caneca` antes de `Ajustar foto`, levando foco à conferência 3D ou abrindo explicitamente o editor, e `Quero que vocês criem`, que coleta foto/frase/orientação e deixa o mockup final com a equipe; editor plano complementar no gabarito confirmado de `21 × 8,7 cm`, com posicionamento por arraste, ações de foto inteira/preenchimento/centralização, zoom, ajuste fino, aviso de resolução e faixas transparentes de proteção de `3%` em cada lateral próxima à alça; máscara equivalente no PNG e na região física sob a alça do modelo 3D; exportação local em PNG de `2480 × 1028 px`/`300 dpi`, com download e compartilhamento nativo quando disponível no modo ajustável; mensagens de WhatsApp específicas para anexar prévia + original ou somente o original e pedir criação; CTAs dos modelos prontos distinguidos do fluxo de personalização, com modelo, SKU e URL preservados na mensagem; opções de `/personalizada` limitadas a `Caneca personalizada` e `Caneca personalizada com foto`, ambas prontas e com personalização inclusa, sem oferta de caneca sem estampa.

**Aceite:** ampliação de produto e prévia 3D operáveis por mouse, teclado e toque; modo ajustável começa com composição automática mais preenchida, oferece conferência 3D com rolagem/foco acessíveis, expõe controles somente após `Ajustar foto` e conserva foto reposicionável por arraste/teclado, presets e zoom refletidos no 3D; deslocamento não abre área vazia dentro do limite sublimável, as faixas da alça permanecem sem tinta no PNG e a arte não aparece sob a alça no 3D; modo assistido não exige editor nem arquivo de prévia, identifica o pedido de criação no WhatsApp e trata o 3D como referência; fallback mantém a personalização utilizável sem WebGL 2; filtros não alteram o catálogo de origem e tratam zero resultados; prévia não persiste nem envia a imagem; PNG exportado tem `2480 × 1028 px`, metadado de `300 dpi` e a mesma composição do editor/3D; cada mensagem do WhatsApp preserva somente os dados pertinentes ao caminho escolhido e ressalva simulação, foto original e aprovação final; modelos prontos usam linguagem de pedido sem sugerir personalização, enquanto `/personalizada` mantém a proposta sob medida e deixa explícito que os valores incluem a personalização; nenhum material, capacidade, foto ou avaliação não confirmada é publicado; typecheck, testes, build, E2E/axe, preview e inspeção desktop/celular aprovados.
...
| 11 — Experiência assistida | Concluída; incremento em preview | Galeria e zoom validados em produção; fluxo híbrido com conferência guiada, prévia 3D, editor opcional no gabarito `21 × 8,7 cm` e handoff específico por WhatsApp do PR `#8`, com fallback 2D, privacidade e acessibilidade verificadas |

## 18. Evoluções incrementais após o roadmap

- **29 de agosto de 2026 — fluxo híbrido, conferência guiada e handoff ao WhatsApp:** `/personalizada` passou a renderizar uma caneca procedural com Three.js e aplicar imagem/frase por textura de canvas mantida somente no navegador. `Quero ver e ajustar` amplia automaticamente a foto em até `140%` quando necessário e apresenta `Ver na caneca` como próxima ação, com rolagem/foco na conferência 3D; `Ajustar foto` abre e fecha explicitamente o editor, cujos controles de arraste, teclado, zoom e presets atualizam o 3D. O gabarito mantém `21 × 8,7 cm`, mas reserva faixas transparentes de `3%` em cada lateral próximas à alça; o posicionamento respeita esses limites, o PNG aplica o mesmo recorte e o 3D mascara a região sob a alça. `Quero que vocês criem` coleta foto, frase e orientação, trata o 3D como referência e leva ao WhatsApp um pedido explícito para a equipe preparar o mockup, sem exigir download. No caminho ajustável, o PNG permanece em `2480 × 1028 px`/`300 dpi` e a mensagem pede prévia + original; no assistido, pede somente o original. Falta de WebGL 2 aciona fallback 2D. A oferta permanece limitada a `Caneca personalizada` e `Caneca personalizada com foto`. O aceite local cobre 79 testes, typecheck, build com 741 URLs, `112/112` cenários Playwright/Axe em Chrome, Firefox, Safari desktop e Safari móvel — incluindo preenchimento inicial, limites laterais, transparência do PNG, dimensões e densidade —, Lighthouse P90+, inspeção desktop/celular e teste privado com arte real do acervo sem copiá-la nem enviá-la. O Edge local repetiu a falha conhecida de encerrar no lançamento e permanece coberto pelo GitHub Actions. O PR em rascunho `#8` mantém preview Vercel protegido por autenticação, sem alterar `main`, produção, banco ou Storage; rollback consiste em restaurar a prévia 2D e retirar fluxo híbrido, editor, exportação e dependências 3D.
