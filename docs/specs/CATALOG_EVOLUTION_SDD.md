...
**Estado:** concluída e validada em produção em 28 de agosto de 2026; simplificação do atendimento assistido publicada em preview protegido pelo PR em rascunho `#8` em 29 de agosto, pendente de aprovação para produção.
...
**Entregas:** zoom acessível de 2,4× recortado na própria caixa e galeria preparada para múltiplas imagens; bloco seguro de detalhes/cuidados; depoimentos moderados no contexto do produto; filtros por tema, coleção e preço com ordenação; `/personalizada` com um único fluxo assistido para escolher a foto original, selecionar o modelo e informar frase/orientações; resumo comercial que explica recebimento, criação do mockup e aprovação antes da produção; compartilhamento nativo do arquivo e de uma mensagem curta somente em celulares cuja Web Share API aceita arquivos; no computador e em celulares incompatíveis, fallback por link de WhatsApp direcionado ao número oficial, com instrução explícita para anexar a imagem manualmente, pois `wa.me` não transporta arquivos; arquivo mantido somente no navegador até a ação do cliente; CTAs dos modelos prontos distinguidos do fluxo de personalização, com modelo, SKU e URL preservados na mensagem; opções de `/personalizada` limitadas a `Caneca personalizada` e `Caneca personalizada com foto`, ambas prontas e com personalização inclusa, sem oferta de caneca sem estampa. O protótipo 3D/editor/exportação foi retirado por decisão do proprietário.

**Aceite:** fluxo assistido sem editor, simulação ou download de prévia; seleção valida JPEG/PNG/WebP de até 10 MB e exibe somente confirmação do arquivo; em celular compatível, compartilhamento nativo inclui o original e um resumo de modelo, arquivo, frase, detalhes, mockup e cidade/CEP; no computador, o CTA usa `wa.me/5548991992341`, abre a conversa oficial com o mesmo resumo e instrui o anexo manual, sem cair no seletor genérico do sistema; somente uma ação principal de envio é exibida por vez; troca/remoção da imagem funciona sem persistência ou upload; filtros não alteram o catálogo de origem e tratam zero resultados; modelos prontos usam linguagem de pedido sem sugerir personalização, enquanto `/personalizada` mantém a proposta sob medida e deixa explícito que os valores incluem a personalização; nenhum material, capacidade, foto ou avaliação não confirmada é publicado; typecheck, testes, build, E2E/axe, preview e inspeção desktop/celular aprovados.
...
| 11 — Experiência assistida | Concluída; simplificação em preview | Galeria e zoom de produto validados em produção; `/personalizada` recebe foto original e orientações em fluxo único, oferece compartilhamento nativo ou instrução de anexo no WhatsApp e deixa o mockup com a equipe no PR `#8` |

## 18. Evoluções incrementais após o roadmap

- **29–30 de agosto de 2026 — personalização assistida sem editor:** por decisão do proprietário, o protótipo híbrido do PR `#8` foi simplificado para um único pedido de criação. O cliente seleciona a foto original, modelo, frase e orientações; a equipe prepara o mockup e o envia para aprovação antes da produção. Em celulares compatíveis, `Enviar foto e pedido pelo WhatsApp` leva arquivo e resumo curto ao seletor do sistema, no qual o cliente escolhe o WhatsApp e a conversa da Criativa. No computador, `Abrir WhatsApp da Criativa` usa o link do número oficial e orienta o anexo manual, sem abrir o compartilhamento genérico do Windows. Não há upload, armazenamento, editor, prévia 2D/3D nem PNG de gabarito. Three.js e os módulos exclusivos da simulação foram removidos; o chunk da rota caiu de aproximadamente `147,7 KB gzip` para `4,3 KB gzip`. O commit `34d03d6` foi aprovado com 69 testes, `108/108` cenários nos navegadores principais, `27/27` no Edge e Lighthouse P90+; a Vercel publicou o preview `Ready`, sem alterar `main` ou produção. Evidência em [`docs/baselines/2026-08-29-assisted-personalization/PERSONALIZATION_HANDOFF.md`](../baselines/2026-08-29-assisted-personalization/PERSONALIZATION_HANDOFF.md).
