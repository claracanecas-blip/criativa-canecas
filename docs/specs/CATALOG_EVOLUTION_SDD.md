...
**Entregas:** zoom acessível de 2,4× recortado na própria caixa e galeria preparada para múltiplas imagens; bloco seguro de detalhes/cuidados; depoimentos moderados no contexto do produto; filtros por tema, coleção e preço com ordenação; prévia local evoluída para caneca 3D com textura dinâmica de imagem/frase, giro por mouse, toque, teclado e botões, zoom e fallback 2D; editor plano complementar no gabarito confirmado de `21 × 8,7 cm`, com posicionamento por arraste, ações de foto inteira/preenchimento/centralização, zoom, ajuste fino e aviso de resolução; exportação local em PNG de `2480 × 1028 px`/`300 dpi`, com download e compartilhamento nativo quando disponível; resumo para WhatsApp com nome da foto, zoom, posição e orientação para anexar prévia e original; CTAs dos modelos prontos distinguidos do fluxo de personalização, com modelo, SKU e URL preservados na mensagem; opções de `/personalizada` limitadas a `Caneca personalizada` e `Caneca personalizada com foto`, ambas prontas e com personalização inclusa, sem oferta de caneca sem estampa.

**Aceite:** ampliação de produto e prévia 3D operáveis por mouse, teclado e toque; foto reposicionável por arraste e teclado, com presets e zoom refletidos no 3D; fallback mantém a personalização utilizável sem WebGL 2; filtros não alteram o catálogo de origem e tratam zero resultados; prévia não persiste nem envia a imagem; PNG exportado tem `2480 × 1028 px`, metadado de `300 dpi` e a mesma composição do editor/3D; mensagem do WhatsApp preserva escolhas, nomeia a prévia e ressalva simulação e necessidade da foto original; modelos prontos usam linguagem de pedido sem sugerir personalização, enquanto `/personalizada` mantém a proposta sob medida e deixa explícito que os valores incluem a personalização; nenhum material, capacidade, foto ou avaliação não confirmada é publicado; typecheck, testes, build, E2E/axe, preview e inspeção desktop/celular aprovados.
...
| 11 — Experiência assistida | Concluída; incremento em preview | Galeria e zoom validados em produção; prévia 3D, editor no gabarito `21 × 8,7 cm` e handoff por PNG/WhatsApp do PR `#8`, com fallback 2D, privacidade e acessibilidade verificadas |

## 18. Evoluções incrementais após o roadmap

- **29 de agosto de 2026 — prévia 3D, enquadramento e handoff ao WhatsApp:** `/personalizada` passou a renderizar uma caneca procedural com Three.js e aplicar imagem/frase por textura de canvas mantida somente no navegador. O cliente enquadra a foto em uma área plana por arraste, teclado, zoom ou ações de foto inteira/preenchimento/centralização; o resultado atualiza o 3D, que gira por mouse, toque, teclado ou botões e aproxima por roda/pinça. O editor foi calibrado para o gabarito operacional confirmado de `21 × 8,7 cm`, sinaliza a resolução estimada e exporta localmente PNG de `2480 × 1028 px` com metadado de `300 dpi`. O cliente pode baixar ou compartilhar a prévia e o WhatsApp recebe nome do original, zoom, posição e nome do arquivo, com orientação para anexar prévia e foto original. Falta de WebGL 2 aciona fallback 2D. A oferta foi simplificada para `Caneca personalizada` e `Caneca personalizada com foto`, removendo mágica e colorida como produtos separados. O aceite local cobriu 77 testes, typecheck, build com 741 URLs, seis cenários direcionados Playwright/Axe em Chrome e Safari móvel — incluindo dimensões e densidade do download — e inspeção desktop/celular. O PR em rascunho `#8` mantém preview Vercel protegido por autenticação, sem alterar `main`, produção, banco ou Storage; rollback consiste em restaurar a prévia 2D e retirar editor, exportação e dependências 3D.
