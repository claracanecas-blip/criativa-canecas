...
- **29–30 de agosto de 2026 — personalização assistida sem editor:** por decisão do proprietário, o protótipo híbrido do PR `#8` foi simplificado para um único pedido de criação. O cliente seleciona a foto original, modelo, frase e orientações; a equipe prepara o mockup e o envia para aprovação antes da produção. Em navegadores compatíveis, `Enviar foto e pedido pelo WhatsApp` leva arquivo e resumo curto ao seletor do sistema, no qual o cliente escolhe o WhatsApp e a conversa da Criativa. O fallback `wa.me` aparece sozinho quando o compartilhamento de arquivos não está disponível e orienta o anexo manual. Não há upload, armazenamento, editor, prévia 2D/3D nem PNG de gabarito. Three.js e os módulos exclusivos da simulação foram removidos; o chunk da rota caiu de aproximadamente `147,7 KB gzip` para `4,3 KB gzip`. O commit `34d03d6` foi aprovado com 69 testes, `108/108` cenários nos navegadores principais, `27/27` no Edge e Lighthouse P90+; a Vercel publicou o preview `Ready`, sem alterar `main` ou produção. Evidência em [`docs/baselines/2026-08-29-assisted-personalization/PERSONALIZATION_HANDOFF.md`](../baselines/2026-08-29-assisted-personalization/PERSONALIZATION_HANDOFF.md).
