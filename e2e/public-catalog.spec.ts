...
test('pedido assistido encaminha a foto e preserva o contexto no WhatsApp', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Caneca personalizada e valores' })).toBeVisible()
  ...
  await expect(page.getByText('Com arte pronta para impressão', { exact: true })).toBeVisible()
  await expect(page.getByText('R$ 39,90', { exact: true })).toBeVisible()
  await expect(page.getByText('Com criação ou adaptação por nós', { exact: true })).toBeVisible()
  await expect(page.getByText('R$ 44,90', { exact: true })).toBeVisible()
  await expect(page.getByText(/Inclui a criação ou adaptação da composição/)).toBeVisible()
  ...
  expect(sharedRequest?.text).toContain('Valor: R$ 39,90 com arte pronta ou R$ 44,90 com criação/adaptação.')
...
test('no computador abre o número da loja sem duplicar a seleção da foto', async ({ page }) => {
  ...
  expect(message).toContain('Valor: R$ 39,90 com arte pronta ou R$ 44,90 com criação/adaptação.')
