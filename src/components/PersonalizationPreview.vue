...
import { CheckCircle2, ImagePlus, MessageCircle, Send, Share2, ShieldCheck, Trash2, WandSparkles } from '@lucide/vue'
...
const requestText = computed(() => {
  const lines = [
    ...
    'Quero que a Criativa Canecas prepare o mockup para eu aprovar antes da produção.',
  ]
  if (imageName.value) {
    lines.push(`Foto original: “${imageName.value}”.`)
    lines.push('Vou anexar a foto original nesta conversa.')
  }
  if (phrase.value.trim()) lines.push(`Texto ou frase: ${phrase.value.trim()}`)
  if (notes.value.trim()) lines.push(`Como imagino a caneca: ${notes.value.trim()}`)
  lines.push('Aguardo o mockup para conferir enquadramento, cores e texto antes da produção.')
...
async function shareOriginal() {
  ...
  try {
    ...
    shareMessage.value = 'Foto e pedido compartilhados. Se você escolheu o WhatsApp, continue a conversa por lá.'
  } catch (error) {
    ...
    <div class="request-layout">
      ...
      <aside class="send-card" aria-labelledby="send-title">
        ...
        <button v-if="canShareOriginal" class="share-original" type="button" :disabled="isSharing" @click="shareOriginal">
          <Share2 :size="19" /> {{ isSharing ? 'Abrindo…' : 'Compartilhar foto e pedido' }}
        </button>
        <p v-if="canShareOriginal" class="share-help">No celular, escolha o WhatsApp na tela de compartilhamento para levar a foto junto.</p>

        <div v-if="canShareOriginal" class="divider"><span>ou</span></div>

        <a class="whatsapp" :href="whatsapp" target="_blank" rel="noopener" @click="trackWhatsappClick('personalized')">
          <MessageCircle :size="20" /> Abrir conversa no WhatsApp
        </a>
        <p class="attachment-note">
          <Send :size="15" />
          <span v-if="imageName"><strong>Ao abrir o WhatsApp, confirme se a foto {{ imageName }} está anexada.</strong> Se não estiver, anexe-a manualmente na conversa.</span>
