...
import { MapPin, MessageCircle, Minus, PackageCheck, Plus, ShoppingBag, Trash2, X } from '@lucide/vue'
...
import { deliveryPolicy, linkWhatsapp, officialSiteOrigin } from '@/data/site'
    ...
    <dialog ref="dialog" class="quote-dialog" aria-labelledby="quote-title" @click="handleBackdrop" @cancel.prevent="close" @close="cart.close">
      <section class="quote-panel">
        ...
        <footer v-if="resolvedItems.length" class="quote-footer">
          ...
          <p class="price-explanation">{{ deliveryPolicy.priceNote }} Personalização, prazo e disponibilidade serão confirmados no atendimento.</p>
          <div class="delivery-summary">
            <p><MapPin :size="16" /><span><strong>{{ deliveryPolicy.local.title }} — {{ deliveryPolicy.local.subtitle }}:</strong> {{ deliveryPolicy.local.description }}</span></p>
            <p><PackageCheck :size="16" /><span><strong>{{ deliveryPolicy.shipping.title }} — {{ deliveryPolicy.shipping.subtitle }}:</strong> {{ deliveryPolicy.shipping.description }}</span></p>
          </div>
          <p>A seleção não reserva estoque nem confirma o pedido.</p>
