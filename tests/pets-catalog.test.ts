...
const fillFixMigration = await readFile(
  new URL('../supabase/migrations/20260820193000_pets_19_mockup_fill_fix.sql', import.meta.url),
  'utf8',
)
const fillFixRollback = await readFile(
  new URL('../supabase/rollback/20260820193000_pets_19_mockup_fill_fix.sql', import.meta.url),
  'utf8',
)
...
test('catálogo local publica exatamente pets-01 a pets-50', () => {
  ...
  assert.equal(petsProducts[18]?.imagem, './img/pets-19-r2.png')
})
...
test('correção do mockup pets-19 usa caminhos versionados e possui rollback', () => {
  for (const path of [
    'pets-19-r2.webp',
    'card/320/pets-19-r2.webp',
    'card/640/pets-19-r2.webp',
    'social/pets-19-r2.webp',
  ]) {
    assert.match(fillFixMigration, new RegExp(path.replaceAll('/', '\\/')))
  }

  assert.match(fillFixRollback, /when 'original' then 'pets-19\.webp'/i)
  assert.doesNotMatch(fillFixRollback, /delete from storage\.objects/i)
})
